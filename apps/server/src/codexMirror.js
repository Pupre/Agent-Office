...
function getNextCoder(mirror, lane = "default") {
  mirror.assignmentCursor ??= {};
  const key = `coder:${lane}`;
  mirror.assignmentCursor[key] = (mirror.assignmentCursor[key] ?? 0) + 1;
  return CODERS[(mirror.assignmentCursor[key] - 1) % CODERS.length];
}

function getCoderSquad(mirror, preferredLeadId) {
  const leadId = preferredLeadId ?? getNextCoder(mirror, "lead");
  const supportIds = CODERS.filter((agentId) => agentId !== leadId);
  return {
    leadId,
    supportIds,
    testerId: TESTERS[0],
...
function getPlanningTeamState(mirror, mode = "kickoff") {
  const assignments = baseAssignments();
  const statuses = baseStatuses();

  if (!mirror || mode === "kickoff") {
  ...
  const squad = getCoderSquad(mirror, getNextCoder(mirror, `planning:${mode}`));
  assignments[PLANNER] = "briefing-room";
  assignments[squad.leadId] = "briefing-room";
  assignments[squad.supportIds[0]] = "build-bay";
  assignments[squad.supportIds[1]] = "qa-lab";
  assignments[squad.testerId] = "qa-lab";
  assignments[squad.reviewerId] = "review-desk";
  statuses[PLANNER] = "planning";
  statuses[squad.leadId] = "planning";
  statuses[squad.supportIds[0]] = "coding";
  statuses[squad.supportIds[1]] = "testing";
  statuses[squad.testerId] = "testing";
  statuses[squad.reviewerId] = "planning";
  return createTeamState(
    [PLANNER, squad.leadId, squad.supportIds[0], squad.supportIds[1], squad.testerId, squad.reviewerId],
    assignments,
    statuses
  );
}

function getWorkTeamState(mirror, primaryStage = "coding") {
  const squad = getCoderSquad(
    mirror,
    mirror ? getNextCoder(mirror, `work:${primaryStage}`) : CODERS[0]
  );
  const assignments = baseAssignments();
  const statuses = baseStatuses();
  assignments[squad.leadId] = "build-bay";
  assignments[squad.supportIds[0]] = primaryStage === "testing" ? "qa-lab" : "build-bay";
  assignments[squad.supportIds[1]] = primaryStage === "retrying" ? "incident-desk" : "build-bay";
  assignments[squad.testerId] = "qa-lab";
  assignments[squad.reviewerId] = "review-desk";
  statuses[squad.leadId] = "coding";
  statuses[squad.supportIds[0]] = primaryStage;
  statuses[squad.supportIds[1]] = primaryStage === "testing" ? "planning" : "coding";
  statuses[squad.testerId] = "testing";
  statuses[squad.reviewerId] = primaryStage === "testing" ? "testing" : "planning";
  return createTeamState([...CODERS, squad.testerId, squad.reviewerId], assignments, statuses);
}

function getReadTeamState(mirror) {
  const squad = getCoderSquad(mirror, getNextCoder(mirror, "read"));
  const assignments = baseAssignments();
  const statuses = baseStatuses();
  assignments[PLANNER] = "briefing-room";
  assignments[squad.leadId] = "briefing-room";
...
function getIncidentTeamState(mirror) {
  const squad = getCoderSquad(mirror, getNextCoder(mirror, "incident"));
  const assignments = baseAssignments();
  const statuses = baseStatuses();
  assignments[squad.leadId] = "incident-desk";
  assignments[squad.supportIds[0]] = "incident-desk";
  assignments[squad.supportIds[1]] = "build-bay";
  assignments[squad.testerId] = "incident-desk";
  assignments[squad.reviewerId] = "review-desk";
  statuses[squad.supportIds[1]] = "coding";
  statuses[squad.leadId] = "retrying";
  statuses[squad.supportIds[0]] = "retrying";
  statuses[squad.testerId] = "failed";
  statuses[squad.reviewerId] = "testing";
  return createTeamState([...CODERS, squad.testerId, squad.reviewerId], assignments, statuses);
}

function getReviewTeamState(mirror) {
  const squad = getCoderSquad(
    mirror,
    mirror ? getNextCoder(mirror, "review") : CODERS[0]
  );
  const assignments = baseAssignments();
  const statuses = baseStatuses();
  assignments[squad.leadId] = "review-desk";
  assignments[squad.supportIds[0]] = "build-bay";
  assignments[squad.supportIds[1]] = "qa-lab";
  assignments[squad.testerId] = "qa-lab";
  assignments[squad.reviewerId] = "review-desk";
  ...
  statuses[squad.supportIds[1]] = "testing";
  statuses[squad.testerId] = "testing";
  statuses[squad.reviewerId] = "success";
  return createTeamState(
    [squad.leadId, squad.supportIds[0], squad.supportIds[1], squad.testerId, squad.reviewerId],
    assignments,
    statuses
  );
}

function getBuildTeamState(mirror, focusCoderId) {
  const squad = getCoderSquad(mirror, focusCoderId ?? getNextCoder(mirror, "build"));
  const assignments = baseAssignments();
  const statuses = baseStatuses();
  const variation =
    ((mirror.assignmentCursor["build:variation"] = (mirror.assignmentCursor["build:variation"] ?? 0) + 1) - 1) % 3;

  assignments[squad.leadId] = "build-bay";
  assignments[squad.supportIds[0]] = variation === 0 ? "build-bay" : variation === 1 ? "qa-lab" : "briefing-room";
  assignments[squad.supportIds[1]] = variation === 2 ? "incident-desk" : "build-bay";
  assignments[squad.testerId] = variation === 1 ? "build-bay" : "qa-lab";
  assignments[squad.reviewerId] = variation === 0 ? "review-desk" : "briefing-room";
  statuses[squad.leadId] = "coding";
  statuses[squad.supportIds[0]] = variation === 2 ? "planning" : variation === 1 ? "testing" : "coding";
  statuses[squad.supportIds[1]] = variation === 2 ? "retrying" : "coding";
  statuses[squad.testerId] = "testing";
  statuses[squad.reviewerId] = variation === 0 ? "testing" : "planning";
  return createTeamState([...CODERS, squad.testerId, squad.reviewerId], assignments, statuses);
}

function detectToolAction(toolName, payload, mirror) {
  if (toolName.includes("apply_patch")) {
    const squad = getCoderSquad(mirror, getNextCoder(mirror, "edit"));
    return {
      agentId: squad.leadId,
      roomId: "build-bay",
      stageId: "coding",
      status: "coding",
      summary: "개발자가 코드 패치를 적용하고 있습니다.",
      detail: "apply_patch를 통해 직접 파일 수정을 진행합니다.",
      teamState: getBuildTeamState(mirror, squad.leadId),
      conversation: {
        agentId: squad.supportIds[0],
        roomId: "build-bay",
        summary: "이 패치 들어가면 옆 모듈 영향도 같이 볼게요.",
        detail: "서포트 개발자가 동시에 여파를 체크합니다."
      }
    };
  }
  ...
  if (toolName.includes("playwright") || toolName.includes("browser_")) {
    return {
      ...
      teamState: getWorkTeamState(mirror, "testing"),
      conversation: {
        agentId: "reviewer-1",
        roomId: "review-desk",
  ...
  if (toolName.includes("search_openai_docs") || toolName.includes("fetch_openai_doc")) {
      return {
        ...
        detail: shortText(JSON.stringify(payload), 180),
        teamState: getReadTeamState(mirror)
      };
  }
  ...
  if (toolName.includes("exec_command")) {
    ...
    if (isRead) {
      return {
        ...
        teamState: getReadTeamState(mirror),
        conversation: {
          agentId: getNextCoder(mirror, "meeting"),
          roomId: "briefing-room",
          summary: "파일 구조를 보고 구현 분리를 제안합니다.",
          detail: "이번에는 다른 개발자가 회의에 끼어들어 구현 분리를 제안합니다."
        }
      };
    }
    ...
    const squad = getCoderSquad(
      mirror,
      isInfra ? getNextCoder(mirror, "incident") : getNextCoder(mirror, isTest ? "test-support" : "build")
    );

    return {
      agentId: isTest ? squad.testerId : squad.leadId,
      ...
      detail: cmd,
      teamState: isTest
        ? getWorkTeamState(mirror, "testing")
        : isInfra
          ? getIncidentTeamState(mirror)
          : getBuildTeamState(mirror, squad.leadId),
      conversation: isInfra
        ? {
            agentId: squad.testerId,
        ...
        : isTest
          ...
          : {
              agentId: squad.supportIds[1],
              roomId: "build-bay",
              summary: "나는 옆 기능도 같이 만져둘게요. 막히면 바로 부를게요.",
              detail: "남은 개발자가 병렬로 다른 슬라이스를 함께 진행합니다."
            }
    };
  }
  ...
  return {
    ...
    detail: shortText(JSON.stringify(payload), 180),
    teamState: getReviewTeamState(mirror)
  };
}
...
export function createCodexMirror({ eventBus, store, codexHome }) {
  ...
  function ensureMirror(sessionId) {
    ...
    const mirror = {
      ...
      title,
      publisher,
      teamState: getPlanningTeamState(null),
      assignmentCursor: {},
  ...
  function ingestHistory(mirror) {
    ...
    for (const entry of entries) {
      ...
      mirror.publisher.emit({
        ...
        detail: entry.text,
        timestamp,
        teamState: getPlanningTeamState(mirror, isFirstPrompt ? "kickoff" : "prompt")
      });
    }
  }
    ...
    for (const line of lines) {
      ...
      if (
        ...
        mirror.publisher.emit({
          ...
          timestamp,
          teamState: getPlanningTeamState(mirror, "kickoff")
        });
        ...
        mirror.publisher.emit({
          ...
          timestamp,
          teamState: getPlanningTeamState(mirror, "prompt")
        });
        continue;
      }
      ...
      if (toolCallMatch) {
        ...
        const action = detectToolAction(toolName, payload, mirror);
        ...
        mirror.publisher.emit({
          ...
          teamState: action.teamState ??
            (action.stageId === "planning"
              ? getPlanningTeamState(mirror, "prompt")
              : getWorkTeamState(mirror, action.stageId === "testing" ? "testing" : "coding"))
        });
      ...
      if (
        ...
        mirror.publisher.emit({
          ...
          timestamp,
          teamState: getIncidentTeamState(mirror)
        });
      }
    }
  }
